	...
	class UpdatePulse_Updater {
		...
		public function load_textdomain() {
			if ( 'Plugin' === $this->type ) {
				load_plugin_textdomain(
					'updatepulse-updater',
					false,
					str_replace( wp_normalize_path( WP_PLUGIN_DIR ) . '/', '', wp_normalize_path( __DIR__ ) ) . '/languages'
				);
			} else {
				load_theme_textdomain( 'updatepulse-updater', __DIR__ . '/languages' );
			}
		}
		...
		public function add_admin_scripts( $hook ) {
			...
			if (
				...
				$ver_js = filemtime( __DIR__ . '/js/main' . $js_ext );
				$params = array(
				...
				wp_enqueue_script(
					...
					plugin_dir_url( __FILE__ ) . 'js/main' . $js_ext,
					array( 'jquery' ),
					$ver_js,
					true
				);
		...
		public function locate_template( $template_name, $load = false, $required_once = true ) {
			$template = apply_filters(
				...
				__DIR__ . '/templates/' . $template_name,
				$template_name,
				__DIR__ . '/templates/'
			);
